...
with tab1:
    ...
    # Display columns
    display_cols = ["_TxID", "Amount", "Time", "_RiskScore", "_RiskLevel", "_Flagged"]
    if "Class" in view_df.columns:
        display_cols.append("Class")

    show = view_df[display_cols].rename(columns={
    ...
    st.dataframe(
        show.style
            ...
            .format({"Amount ($)": "{:.2f}", "Risk Score": "{:.1f}", "Time (s)": "{:.0f}"}),
    ...
    if n_flagged:
        ...
        for _, row in top_alerts.iterrows():
            ...
            <div class="alert-box">
                ...
                <div class="tx-detail">
                    Amount: ${float(amt):.2f} · Time: {float(row.get('Time',0)):.0f}s{true_lbl}
                </div>
            </div>""", unsafe_allow_html=True)
